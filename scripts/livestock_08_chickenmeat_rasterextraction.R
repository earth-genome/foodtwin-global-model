...
lv<-'glw/chickens/5_Ch_2015_Da.tif' 
lv_raster=raster(lv)
...
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 21, 23, 24, 118, 121, 153, 160, 161, 167, 168, 175), 0, totalanimaldens_15))
...
final_lookup3 <- final_lookup2 %>% filter(Animal=="chickenmeat")
final_lookup3_dens <- final_lookup3 %>% filter(Unit=="1000 An")
...
multi <- final_lookup3_dens$Value_20*1000

final_lookup3_vol <- final_lookup3 %>% filter(Unit=="0.1 g/An")
...
energy_lookup2 <- energy_lookup %>% filter(Animal=="chickenmeat")
...
lv_raster_new2=lv_raster

for(i in 1:length(animals_dens)) {
  lv_raster_new2[animals_dens[[i]][,1]] <- animals_dens[[i]][,2] 
}
...
summaries$animal <- "chickenmeat"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_chickenmeat_summary_noloss.csv"))
...
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_chickenmeat_annual_production_noloss.tif')
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_chickenmeat_annual_calories_noloss.tif')
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_chickenmeat_annual_fats_noloss.tif')
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_chickenmeat_annual_protein_noloss.tif')



my_window <- extent(100, 120, 20, 40)
plot(my_window, col=NA)
plot(lv_raster, add=T)
plot(lv_raster_calories, add=T)
plot(lv_raster_new2, add=T)
...
sum15_2_sum <- cbind(countrylist, sum15_2_sum)
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 21, 23, 24, 116, 160, 161, 167, 168, 175, 177), 0, totalanimaldens_15))
frac <- sum15_2_sum$totalanimaldens_15
...
#divide multiplier to go from 100g/an to tonnage
final_lookup3 <- final_lookup2 %>% filter(Animal=="eggs")
final_lookup3_dens <- final_lookup3 %>% filter(Unit=="1000 An")
countrylist$num <- seq.int(nrow(countrylist))
final_lookup3_dens <- left_join(countrylist, final_lookup3_dens, by="ISO_N3_EH")
final_lookup3_dens <- final_lookup3_dens %>%
...
final_lookup3_vol <- final_lookup3 %>% filter(Unit=="100 mg/An")
final_lookup3_vol <- left_join(countrylist, final_lookup3_vol, by="ISO_N3_EH")
final_lookup3_vol <- final_lookup3_vol %>%
  mutate(Value_20 = if_else(is.na(Value_20), 0, Value_20))
multi2 <- final_lookup3_vol$Value_20
...
#also convert to calories, fats and proteins (convert from per 100g to 1T)
energy_lookup2 <- energy_lookup %>% filter(Animal=="eggs")
cal <- energy_lookup2$calories[1]
calories <- animals_vol
for (i in 1:length(calories)) {
  calories[[i]][,2] <- calories[[i]][,2]*1e+4*cal
...
summaries$totalfats_20 <- summaries$totalanimalvol_20*1e+4*fat
summaries$totalprt_20 <- summaries$totalanimalvol_20*1e+4*prt
summaries$animal <- "eggs"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_eggs_summary_noloss.csv"))


#write rasters to file
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_eggs_annual_production_noloss.tif', overwrite=TRUE)
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_eggs_annual_calories_noloss.tif', overwrite=TRUE)
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_eggs_annual_fats_noloss.tif',overwrite=TRUE)
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_eggs_annual_protein_noloss.tif', overwrite=TRUE)


my_window <- extent(65, 85, 10, 40)
plot(my_window, col=NA)
plot(lv_raster, add=T)
