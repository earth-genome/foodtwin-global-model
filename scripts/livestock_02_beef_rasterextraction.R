...
lv<-'input_data/glw/cattle/5_Ct_2015_Da.tif' 
lv_raster=raster(lv)
#lv_raster
...
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 21, 23, 24, 102, 160, 161, 168, 175), 0, totalanimaldens_15))
...
final_lookup3 <- final_lookup2 %>% filter(Animal=="beef")
...
energy_lookup2 <- energy_lookup %>% filter(Animal=="beef")
...
# for(i in 1:length(animals_dens)) {
#   lv_raster_newdens[animals_dens[[i]][,1]] <- animals_dens[[i]][,2] 
# }
...
#summaries <- left_join(country_list, summaries, by="num")
...
summaries$animal <- "beef"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_beef_summary_noloss.csv"))


#write rasters to file
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_beef_annual_production_noloss.tif', overwrite=TRUE)
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_beef_annual_calories_noloss.tif', overwrite=TRUE)
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_beef_annual_fats_noloss.tif', overwrite=TRUE)
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_beef_annual_protein_noloss.tif', overwrite=TRUE)
...
#extract cell numbers from polygon overlay on raster
lv<-'glw/cattle/5_Ct_2015_Da.tif' 
lv_raster=raster(lv)
#lv_raster
animals_dens <- raster::extract(lv_raster,sf_as_sp,cellnumbers=TRUE)
#test <- animals_dens
...
sum15_2_sum <- cbind(countrylist, sum15_2_sum)
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 21, 23, 24, 25, 70, 160, 161, 168, 175), 0, totalanimaldens_15))
frac <- sum15_2_sum$totalanimaldens_15
...
#divide multiplier to go from 100g/an to tonnage
final_lookup3 <- final_lookup2 %>% filter(Animal=="cowsmilk")
final_lookup3_dens <- final_lookup3 %>% filter(Unit=="An")
countrylist$num <- seq.int(nrow(countrylist))
final_lookup3_dens <- left_join(countrylist, final_lookup3_dens, by="ISO_N3_EH")
final_lookup3_dens <- final_lookup3_dens %>%
...
#also convert to calories, fats and proteins (convert from per 100g to 1T)
energy_lookup2 <- energy_lookup %>% filter(Animal=="cowsmilk")
cal <- energy_lookup2$calories[1]
calories <- animals_vol
for (i in 1:length(calories)) {
  calories[[i]][,2] <- calories[[i]][,2]*1e+4*cal
...
summaries$totalfats_20 <- summaries$totalanimalvol_20*1e+4*fat
summaries$totalprt_20 <- summaries$totalanimalvol_20*1e+4*prt
summaries$animal <- "cowsmilk"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_cowsmilk_summary_noloss.csv"))


#write rasters to file
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_cowsmilk_annual_production_noloss.tif')
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_cowsmilk_annual_calories_noloss.tif')
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_cowsmilk_annual_fats_noloss.tif')
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_cowsmilk_annual_protein_noloss.tif')
