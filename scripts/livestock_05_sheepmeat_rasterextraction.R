...
world <- sf::read_sf("input_datane_110m_admin_0_countries/ne_110m_admin_0_countries.shp")
sf_as_sp <- sf::as_Spatial(world)

#extract cell numbers from polygon overlay on raster
lv<-'input_dataglw/sheep/5_Sh_2015_Da.tif' 
lv_raster=raster::raster(lv)
lv_raster
...
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 21, 23, 24, 34, 90, 91, 93, 95, 136, 141, 160, 161, 168, 175), 0, totalanimaldens_15))
...
final_lookup <- read.csv("input_datafaostat_livmultipliers2020_total.csv")
energy_lookup <- read.csv(("input_dataprod_energylookup.csv"))
...
final_lookup3 <- final_lookup2 %>% filter(Animal=="sheepmeat")
...
summaries$animal <- "sheepmeat"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_sheepmeat_summary_noloss.csv"))

plot(lv_raster)
plot(lv_raster_newvol)

#write rasters to file
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_sheepmeat_annual_production_noloss.tif')
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_sheepmeat_annual_calories_noloss.tif')
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_sheepmeat_annual_fats_noloss.tif')
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_sheepmeat_annual_protein_noloss.tif')



my_window <- extent(75, 85, 30, 50)
plot(my_window, col=NA)
plot(lv_raster, add=T)
plot(lv_raster_newvol, add=T)
...
# 
# #extract cell numbers from polygon overlay on raster
# lv<-'glw/sheep/5_Sh_2015_Da.tif' 
# lv_raster=raster(lv)
# animals_dens <- raster::extract(lv_raster,sf_as_sp,cellnumbers=TRUE)
# 
# 
# #get list of coutest#get list of country polygons
# countrylist <- world %>% dplyr::select(ISO_N3_EH, SOVEREIGNT)
# countrylist <- sf::st_drop_geometry(countrylist)
# 
# #create summaries by country of total production volumes
...
# sum15_2_sum <- cbind(countrylist, sum15_2_sum)
# sum15_2_sum <- sum15_2_sum %>%
#   mutate(totalanimaldens_15 = if_else(num %in% c(1, 3, 4, 7, 10, 12, 18, 21, 23, 24, 26, 27, 29, 33, 34, 36, 37, 38, 
#                                                  40, 41, 42, 43, 49, 51, 57, 59, 60, 61, 64, 65, 67, 68, 69, 70, 71, 
#                                                  74, 75, 79, 81, 90, 91, 92, 93, 95, 96, 101, 107, 111, 114, 121, 122, 
#                                                  129, 130, 134, 135, 136, 137, 138, 143, 144, 145, 148, 149, 150, 152, 
#                                                  153, 154, 156, 157, 160, 161, 167, 168, 169, 171, 175, 176), 0, totalanimaldens_15))
# frac <- sum15_2_sum$totalanimaldens_15
# 
# #apply fractionater to each cell of each country polgy to get fraction of cattle density
# for (i in 1:length(animals_dens)) {
#   animals_dens[[i]][,2] <- animals_dens[[i]][,2]/frac[i]
# }
# 
# 
# #join lookup tables to country shapefiles
# final_lookup <- read.csv("faostat_livmultipliers2020_total.csv")
...
# 
# 
# #filter joint shapefile table for product of interest, extract multipliers
# #divide multiplier to go from 100g/an to tonnage
# final_lookup3 <- final_lookup2 %>% filter(Animal=="sheepsmilk")
# final_lookup3_dens <- final_lookup3 %>% filter(Unit=="An")
# countrylist$num <- seq.int(nrow(countrylist))
# final_lookup3_dens <- left_join(countrylist, final_lookup3_dens, by="ISO_N3_EH")
# final_lookup3_dens <- final_lookup3_dens %>%
...
# 
# #also convert to calories, fats and proteins (convert from per 100g to 1T)
# energy_lookup2 <- energy_lookup %>% filter(Animal=="goatmilk")
...
# summaries$totalfats_20 <- summaries$totalanimalvol_20*1e+4*fat
# summaries$totalprt_20 <- summaries$totalanimalvol_20*1e+4*prt
# summaries$animal <- "goatmilk"
# summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
# write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_goatmilk_summary_noloss.csv"))
# 
# 
# #write rasters to file
# writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_goatmilk_annual_production_noloss.tif')
# writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_goatmilk_annual_calories_noloss.tif')
# writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_goatmilk_annual_fats_noloss.tif')
# writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_goatmilk_annual_protein_noloss.tif')
# 
# 
# my_window <- extent(50, 100, 0, 50)
# plot(my_window, col=NA)
# plot(lv_raster, add=T)
# plot(lv_raster_newvol, add=T)
