...
lv<-'input_data/glw/goats/5_Gt_2015_Da.tif' 
lv_raster=raster(lv)
...
sum15_2_sum <- sum15_2_sum %>%
  mutate(totalanimaldens_15 = if_else(num %in% c(3, 4, 21, 23, 24, 29, 34, 91, 105, 112, 117, 136, 144, 145, 146, 160, 161, 168, 172, 175), 0, totalanimaldens_15))
...
final_lookup3 <- final_lookup2 %>% filter(Animal=="goatmeat")
...
energy_lookup2 <- energy_lookup %>% filter(Animal=="goatmeat")
...
summaries$animal <- "goatmeat"
summaries <- do.call(data.frame,lapply(summaries, function(x) replace(x, is.infinite(x),0)))
write_csv(summaries, paste0("2020_livestocksummaries/noloss/2020_goatmeat_summary_noloss.csv"))

plot(lv_raster)
plot(lv_raster_newvol)

#write rasters to file
writeRaster(lv_raster_newvol, '2020_livestockrasters/annual/noloss/tonnage/individualitem/2020_goatmeat_annual_production_noloss.tif')
writeRaster(lv_raster_calories, '2020_livestockrasters/annual/noloss/calories/individualitem/2020_goatmeat_annual_calories_noloss.tif')
writeRaster(lv_raster_fats, '2020_livestockrasters/annual/noloss/fats/individualitem/2020_goatmeat_annual_fats_noloss.tif')
writeRaster(lv_raster_prt, '2020_livestockrasters/annual/noloss/protein/individualitem/2020_goatmeat_annual_protein_noloss.tif')
...
#divide multiplier to go from 100g/an to tonnage
final_lookup3 <- final_lookup2 %>% filter(Animal=="goatmilk")
final_lookup3_dens <- final_lookup3 %>% filter(Unit=="An")
countrylist$num <- seq.int(nrow(countrylist))
final_lookup3_dens <- left_join(countrylist, final_lookup3_dens, by="ISO_N3_EH")
final_lookup3_dens <- final_lookup3_dens %>%
